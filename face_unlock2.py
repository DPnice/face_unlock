...
def spot_face():
    ...
    while cap.isOpened():
        if f > 10:
            cap.release()
            messagebox.showerror('你想干啥', '我觉得你可以再试一次 d=====(￣▽￣*)b')
        cf += 100
        ...
        if kk == ord('q'):
            ...
            if len(faces) != 0:
                ...
                for k in range(len(faces)):
                    ...
                    if len(name_namelist) < len(faces):
                    ...
                    if len(features_cap_arr) > 0:
                        ...
                        if min(e_distance_list) < 0.4:
                            ...
                            name_namelist[k] = "DPnice " + str(int(similar_person_num) + 1)
...
class Application(Frame):
    ...
    def createWidgets(self):
        self.tiplabel = Label(self, text='输入“密码”')
        self.tiplabel.pack()
        self.valueInput = Entry(self, text='input your pwd', show='*')
        self.valueInput.pack()
        self.alertButton = Button(self, text='验证', command=self.proof)
        self.alertButton.pack()

        self.action = Button(self, text="Click Me!人脸识别", command=spot_face)
        self.action.pack()
...
# 背景
# canvas = tkinter.Canvas(root, width=w, height=h/2, bd=0, highlightthickness=0)
